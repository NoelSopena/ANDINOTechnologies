...
<html>
	<head>
		...
		<script type="text/JavaScript">
			function valid(f) {
				!(/^[A-z;0-9; ;.;,;#;&;-;ñ]*$/i).test(f.value)?f.value = f.value.replace(/[^A-z0-9; ;.;,;#;&;-;ñ]/ig,''):null;
			} 
		</script>
		<script>
	...
	<body>
		...
		<div class="container">
			...
			<br></br>
			<form id="form_secretaryViewCaseUpdate" action="secretaryViewCaseUpdate.php" method="post" enctype="multipart/form-data">
				<?php
					...
					*/
					...
					CONVERT(VARCHAR(11),Deadline,21) AS fechaLimite, Precedence, DocDescription, Appellant, Sender, DocType FROM Documents WHERE DocNumber = '$_SESSION[docID]'";
				...
				<div class="row">
					...
					<div class="col-md-5">
						...
						<input type="text" class="form-control" placeholder="<?php echo $lang['genSubcategory']; ?>" onkeyup="valid(this)" onblur="valid(this)" name="DocSubcategory" id="DocSubcategory" value="<?php foreach ($subcategory as $key) { echo $key . ", "; } ?>" >
					</div>
					...
					<?php
						if ($titulo == 'Demanda') {
							...
							echo "<label 'control-label'> $lang[case_amount]:</label><input type='number' step='0.01' class='form-control', placeholder='&#36 ".$row['Amount']."' value='".$row['Amount']."' name='quantity' id='quantity'>";
							echo "</div>";
						}
					?>
				</div>
				...
				<div class="row">
					...
					<div class="col-md-3">
						...
						<input type="text" class="form-control" placeholder="<?php echo $lang['case_sender']; ?>" onkeyup="valid(this)" onblur="valid(this)" name="Sender" id="Sender" value='<?php echo $sender ?>'>
					</div>
					<div class="col-md-6">
